...
func _init(team_home, team_away) -> void:
...
func increase_score(country_scored_on) -> void:
...
func update_match_info() -> void:
	winner = country_away if goals_away > goals_home else country_home
	final_score = "%d - %d" % [max(goals_home, goals_away), min(goals_home, goals_away)]


func has_someone_scored() -> bool:
	return goals_away > 0 or goals_home> 0
